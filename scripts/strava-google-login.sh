...
# Handles the full multi-step flow: login → 2FA selection → phone tap / OTP
#
# Usage: ./scripts/strava-google-login.sh EMAIL PASSWORD
...
echo "Logging in via Google OAuth..."
RESULT=$(curl -s -X POST "${BASE}/auth/login-with-credentials" \
  -H "Content-Type: application/json" \
  -d "{\"email\": \"${EMAIL}\", \"password\": \"${PASSWORD}\", \"method\": \"google\"}")
