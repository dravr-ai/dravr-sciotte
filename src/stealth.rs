...
/// Inject the stealth payload into a page. Must be called after `new_page`
/// and before any navigation that should appear non-automated. Runs on every
/// frame creation thereafter, including subsequent `page.goto(...)` calls.
pub async fn apply_minimal_stealth(page: &chromiumoxide::Page) -> ScraperResult<()> {
